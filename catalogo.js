fetch('https://diwserver.vps.webdock.cloud/products/search?query=1&page_items=12')
    .then(res => res.json())
    .then(data => {
        let str = '';
        for (let i = 0; i < data.length; i++) {
            ...
            let rating = produto.rating.rate;
            ...
            <div class="col-xxl-3 col-xl-6 col-lg-6 col-md-6 col-sm-6 ">
                <div class="card-n m-0 mb-4" data-aos="fade-up" data-aos-duration="1000">
                    <a class='main' href="detalhes.html?id=${produto.id}">
                        <div id="card-product-1"></div>
                        <img class='tokenImage' src="${produto.image}" alt="${produto.title}"/>
                        <h2 class="text-white mt-3 title-card">${produto.title}</h2>
                        <div class="mt-2 mb-2" style="display: flex; align-items: center;">
                            <span class="text-white fonte-serrat">${star}</span>
                            <span class="fonte-serrat">(${produto.rating.count})</span>
                        </div>
                        <p class='description'>Brand: ${produto.brandName}</p>
                        <div class='tokenInfo'>
                            <div class="price">
                                <p>U$${produto.price}</p>
                            </div>
